...
author: "Mirijam Schaaf and Cora Burgwinkel"
output: 
  html_document
---
...
version or R: 3.5.3 "Great Truth"
...
that were given orange juice.

```{r tidy = FALSE, message = FALSE, warning = FALSE, error = FALSE}

guinea_pigs_OJ <- subset(ToothGrowth, supp== "OJ")
print(guinea_pigs_OJ)
...
guinea_pigs_VC <- subset(ToothGrowth, supp== "VC" & dose>= 1)
print(guinea_pigs_VC)
...
group_by(iris_tibble, supp) %>% summarise(mean=mean(len))
...
iris_tibble %>% ggplot(aes(dose, len)) + geom_point(aes(color = supp))
...
As we have already seen when calculating the mean, guinea pigs which received orange juice have longer teeth on average. At a dose of 2.0, the teeth length of guinea pigs which received ascorbic acid is more widely spread. Of corse, the plot only depicts the observations and no causal interpretation is possible or allowed here.
